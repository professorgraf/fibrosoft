...
# class UNet(nn.Module):
#     # for compatibility enc_channels=(3, 16, 32, 64), dec_channels=(64, 32, 16),
#     #                   nb_classes=3, retain_dim=True,
#     #                   out_size=(INPUT_IMAGE_HEIGHT, INPUT_IMAGE_WIDTH)
#     def __init__(self, enc_channels=(3, 64, 128, 256, 512, 1024), dec_channels=(1024, 512, 256, 128, 64), nb_classes=3,
#                  retain_dim=True, out_size=(INPUT_IMAGE_HEIGHT, INPUT_IMAGE_WIDTH)):
#         # maxim
#         super().__init__()
...
#         padding = 0
#         # Encoder
#         # In the encoder, convolutional layers with Conv2d function are used to extract features from an input image.
#         # Each block in the encoder consists of two convolutional layers followed by a max-pooling layer, except
#         # the last block which does not include a max-pooling layer.
#         # -------
#         # input: 572x572x3
#         # ENC_CHANNELS[0], ENC_CHANNELS[1]
#         self.e11 = nn.Conv2d(enc_channels[0], enc_channels[1], kernel_size=3, padding=padding)  # output: 570x570x64
#         self.bn11 = nn.BatchNorm2d(64)
#         self.e12 = nn.Conv2d(enc_channels[1], enc_channels[1], kernel_size=3, padding=padding)  # output: 568x568x64
#         self.bn12 = nn.BatchNorm2d(enc_channels[1])
#         self.pool1 = nn.MaxPool2d(kernel_size=2, stride=2)  # output: 284x284x64
#
#         # input: 284x284x64
#         self.e21 = nn.Conv2d(enc_channels[1], enc_channels[2], kernel_size=3, padding=padding)  # output: 282x282x128
#         self.bn21 = nn.BatchNorm2d(enc_channels[2])
#         self.e22 = nn.Conv2d(enc_channels[2], enc_channels[2], kernel_size=3, padding=padding)  # output: 280x280x128
#         self.bn22 = nn.BatchNorm2d(enc_channels[2])
#         self.pool2 = nn.MaxPool2d(kernel_size=2, stride=2)  # output: 140x140x128
#
#         # input: 140x140x128
#         self.e31 = nn.Conv2d(enc_channels[2], enc_channels[3], kernel_size=3, padding=padding)  # output: 138x138x256
#         self.bn31 = nn.BatchNorm2d(enc_channels[3])
#         self.e32 = nn.Conv2d(enc_channels[3], enc_channels[3], kernel_size=3, padding=padding)  # output: 136x136x256
#         self.bn32 = nn.BatchNorm2d(enc_channels[3])
#         self.pool3 = nn.MaxPool2d(kernel_size=2, stride=2)  # output: 68x68x256
#
#         # input: 68x68x256
#         self.e41 = nn.Conv2d(enc_channels[3], enc_channels[4], kernel_size=3, padding=padding)  # output: 66x66x512
#         self.bn41 = nn.BatchNorm2d(enc_channels[4])
#         self.e42 = nn.Conv2d(enc_channels[4], enc_channels[4], kernel_size=3, padding=padding)  # output: 64x64x512
#         self.bn42 = nn.BatchNorm2d(512)
#         self.pool4 = nn.MaxPool2d(kernel_size=2, stride=2)  # output: 32x32x512
#
#         # input: 32x32x512
#         self.e51 = nn.Conv2d(enc_channels[4], enc_channels[5], kernel_size=3, padding=padding)  # output: 30x30x1024
#         self.bn51 = nn.BatchNorm2d(enc_channels[5])
#         self.e52 = nn.Conv2d(enc_channels[5], enc_channels[5], kernel_size=3, padding=padding)  # output: 28x28x1024
#         self.bn52 = nn.BatchNorm2d(enc_channels[5])
#
#         # Decoder
#         self.upconv1 = nn.ConvTranspose2d(dec_channels[0], DEC_CHANNELS[1], kernel_size=2, stride=2)
#         self.d11 = nn.Conv2d(dec_channels[0], dec_channels[1], kernel_size=3, padding=padding)
#         self.bnd11 = nn.BatchNorm2d( dec_channels[1])
#         self.d12 = nn.Conv2d( dec_channels[1],  dec_channels[1], kernel_size=3, padding=padding)
#         self.bnd12 = nn.BatchNorm2d( dec_channels[1])
#
#         self.upconv2 = nn.ConvTranspose2d( dec_channels[1],  dec_channels[2], kernel_size=2, stride=2)
#         self.d21 = nn.Conv2d( dec_channels[1],  dec_channels[2], kernel_size=3, padding=padding)
#         self.bnd21 = nn.BatchNorm2d( dec_channels[2])
#         self.d22 = nn.Conv2d(dec_channels[2], dec_channels[2], kernel_size=3, padding=padding)
#         self.bnd22 = nn.BatchNorm2d(dec_channels[2])
#
#         self.upconv3 = nn.ConvTranspose2d(dec_channels[2], dec_channels[3], kernel_size=2, stride=2)
#         self.d31 = nn.Conv2d(dec_channels[2], dec_channels[3], kernel_size=3, padding=padding)
#         self.bnd31 = nn.BatchNorm2d(dec_channels[3])
#         self.d32 = nn.Conv2d(dec_channels[3], dec_channels[3], kernel_size=3, padding=padding)
#         self.bnd32 = nn.BatchNorm2d(dec_channels[3])
#
#         self.upconv4 = nn.ConvTranspose2d(dec_channels[3], dec_channels[4], kernel_size=2, stride=2)
#         self.d41 = nn.Conv2d(dec_channels[3], dec_channels[4], kernel_size=3, padding=padding)
#         self.bnd41 = nn.BatchNorm2d(dec_channels[4])
#         self.d42 = nn.Conv2d(dec_channels[4], dec_channels[4], kernel_size=3, padding=padding)
#         self.bnd42 = nn.BatchNorm2d(dec_channels[4])
#
#         # Output layer
#         self.outconv = nn.Conv2d(dec_channels[4], nb_classes, kernel_size=1)
#         self.bn_out = nn.BatchNorm2d(nb_classes)
#
#     def crop(self, enc_features, x_shape):
#         # grab the dimensions of the inputs, and crop the encoder
#         # features to match the dimensions
#         (_, _, H, W) = x_shape
#         enc_features = CenterCrop([H, W])(enc_features)
...
#         xu1 = F.interpolate(xu1, xe42.shape[2:])
#         xu11 = torch.cat([xu1, self.crop(xe42, xu1.shape)], dim=1)
#         xd11 = relu(self.bnd11(self.d11(xu11)))
#         xd12 = relu(self.bnd12(self.d12(xd11)))
#
#         xu2 = self.upconv2(xd12)
#         xu2 = F.interpolate(xu2, xe32.shape[2:])
#         xu22 = torch.cat([xu2, self.crop(xe32, xu2.shape)], dim=1)
#         xd21 = relu(self.bnd21(self.d21(xu22)))
#         xd22 = relu(self.bnd22(self.d22(xd21)))
#
#         xu3 = self.upconv3(xd22)
#         xu3 = F.interpolate(xu3, xe22.shape[2:])
#         xu33 = torch.cat([xu3, self.crop(xe22, xu3.shape)], dim=1)
#         xd31 = relu(self.bnd31(self.d31(xu33)))
#         xd32 = relu(self.bnd32(self.d32(xd31)))
#
#         xu4 = self.upconv4(xd32)
#         xu4 = F.interpolate(xu4, xe12.shape[2:])
#         xu44 = torch.cat([xu4, self.crop(xe12, xu4.shape)], dim=1)
#         xd41 = relu(self.bnd41(self.d41(xu44)))
#         xd42 = relu(self.bnd42(self.d42(xd41)))
#
#         # Output layer
#         out = self.bn_out(self.outconv(xd42))
#
#         if self.retain_dim:
